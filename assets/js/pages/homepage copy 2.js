    ...
    $(document).ready(function () {
        ...
        function updateSelectedValues(add, category, value) {
            if (add) {
                ...
                // Vérifiez si selectedValues[category] est un tableau
                if (Array.isArray(selectedValues[category])) {
                    const index = selectedValues[category].findIndex(item => item.id === value.id);
                    if (index > -1) {
                        // Supprimer la valeur si elle est trouvée
                        selectedValues[category].splice(index, 1);
                    }
                } else {
                    console.log('selectedValues[' + category + '] doit être un tableau');
                }
            }
                            ...
                            if (this.id === 'released-add' || this.id === 'rating-add') {
                                ...
                                removeButtons.forEach(removeButton => {
                                    removeButton.addEventListener('click', function () {
                                        ...
                                        updateSelectedRatingAndRelasedValues('rating', null, null);
                                        updateSelectedRatingAndRelasedValues('released', null, null);
                                        sendSelectedValues();
                                    });
                                });
                            }
